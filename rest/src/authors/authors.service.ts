...
export class AuthorsService {
  private authors: any; //Author[] = authors;
  private fuse:any;
  ...
  async init() {
   ...
   // const fuse = new Fuse(productsTemp, options);
    this.authors = await this.db.collection('authors').find().toArray();
    this.fuse = new Fuse(this.authors, options);
   // console.log("Database===>"+JSON.stringify(this.authors ));
