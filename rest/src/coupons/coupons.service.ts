...
export class CouponsService {
  private coupons: any; // Coupon[] = coupons;
  private fuse:any;
  constructor(
  ...
  async init() {
   ...
   // const fuse = new Fuse(productsTemp, options);
    this.coupons = await this.db.collection('coupons').find().toArray();
    this.fuse = new Fuse(this.coupons, options);
    //console.log("Database===>"+JSON.stringify(this.coupons ));
